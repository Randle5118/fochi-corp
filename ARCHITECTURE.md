...
| `COPY.md` | サイトに載せる**日本語テキスト**（文案の真実の源・禁止語） | — |

→ 同じ内容を複数か所に書かない。鉄則の要約のみ各所に置き、本体は本書／`CLAUDE.md`／`COPY.md` に集約する。
...
- 全ページ共通の背景（`components/SiteBackdrop.tsx` の区画図）も同じトークンに従う。
  canvas には Tailwind のクラスを当てられないため、色は同ファイル冒頭の `COLOR` 定数に
  1か所だけ置き、`tailwind.config.ts` の `border` / `primary` と同期させる。
...
| 2026-09-07 | 背景を全ページ共通の fixed canvas とし、時間ループではなくスクロール連動で描画 | 常時アニメーションはモバイル性能に影響するため |
